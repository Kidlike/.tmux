...
_apply_theme() {
  ...
  tmux_conf_theme_window_status_format=$(echo "$tmux_conf_theme_window_status_format" | sed 's%#{circled_window_index}%#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _circled_digit #I)%g')
  tmux_conf_theme_window_status_current_format=$(echo "$tmux_conf_theme_window_status_current_format" | sed 's%#{circled_window_index}%#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _circled_digit #I)%g')
  ...
  tmux_conf_theme_status_left=$(echo "$tmux_conf_theme_status_left" | sed \
    -e "s%#{root}%#[fg=$tmux_conf_theme_root_fg]#[bg=$tmux_conf_theme_root_bg]#[$tmux_conf_theme_root_attr]#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _root #{pane_tty} #D)#[inherit]%g")
...
EOF
  ...
  case "$status_left $status_right" in
    *'#{battery_status}'*|*'#{battery_bar}'*|*'#{battery_hbar}'*|*'#{battery_vbar}'*|*'#{battery_percentage}'*)
      ...
      status_right="#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _battery)$status_right"
      ;;
  esac
  ...
  case "$status_left $status_right" in
    *'#{username}'*|*'#{hostname}'*|*'#{username_ssh}'*|*'#{hostname_ssh}'*)
      status_left=$(echo "$status_left" | sed \
        -e 's%#{username}%#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _username #{pane_tty} false #D)%g' \
        -e 's%#{hostname}%#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _hostname #{pane_tty} false #D)%g' \
        -e 's%#{username_ssh}%#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _username #{pane_tty} true #D)%g' \
        -e 's%#{hostname_ssh}%#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _hostname #{pane_tty} true #D)%g')
  ...
  case "$status_left $status_right" in
    *'#{uptime_d}'*|*'#{uptime_h}'*|*'#{uptime_m}'*)
      ...
      status_right=$(echo "$status_right" | sed -E \
        ...
        -e 's/#\{(\?)?uptime_m/#\{\1@uptime_m/g' \
        -e 's/#\{(\?)?uptime_s/#\{\1@uptime_s/g')
      status_right="#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _uptime)$status_right"
      ;;
  esac
  ...
  case "$status_left $status_right" in
    *'#{loadavg}'*)
      ...
      status_right=$(echo "$status_right" | sed -E \
        -e 's/#\{(\?)?loadavg/#\{\1@loadavg/g')
      status_right="#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _loadavg)$status_right"
      ;;
  esac

  status_left=$(echo "$status_left" | sed 's%#{circled_session_name}%#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _circled_digit #S)%g')
  status_right=$(echo "$status_right" | sed 's%#{circled_session_name}%#(cat ~/.tmux/oh-my-tmux.sh | sh -s|cat ~/.tmux/oh-my-tmux.sh | sh -s _circled_digit #S)%g')
